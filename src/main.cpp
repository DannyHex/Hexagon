...
#if defined(NDEBUG)
# error "Hexagon cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xc3b393267c3f94b75726861d35347a4177e44b1f9cdeb969e2a056fe2d6c4417");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Hexagon: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Hexagon Signed Message:\n";
...
{
    // Hexagon: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 3141 * COIN;

            if(nHeight == 1)
            {
            nSubsidy = 53301818 * COIN;
            }
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Hexagon: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Hexagon: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1497429610, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "Hexagon Kicks Off HxGN LIVE Conference in Las Vegas";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 3141 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0444087b9a5ed909f8444923138417f93b8a9814aa80c3626c9be4cf12ebf61fca24a110c9d18958d33dd621a0305b24a2fd1cf59b3a20a20c45c059d6a9d9cd32") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1497429610;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1033053;
        ...
        assert(block.hashMerkleRoot == uint256("0x3766160ac2cf63d5b9cc13bee5319e18f2d58ee6b1bfdb7f3692f80f00a134ca"));
...
unsigned char pchMessageStart[4] = { 0xe8, 0xdc, 0xb0, 0x78 }; // Hexagon: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("HexagonMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("HexagonMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static HexagonMiner(CWallet *pwallet)
{
    printf("HexagonMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("hexagon-miner");
    ...
    try { loop {
        ...
        printf("Running HexagonMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("HexagonMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&HexagonMiner, pwallet));
}
